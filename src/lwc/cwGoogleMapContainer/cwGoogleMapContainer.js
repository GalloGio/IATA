...
export default class GoogleMapContainer extends LightningElement {
	...
	@api mapOptions; 
	mapDataLocalName;
	companyTypeFilterLocalNameToAvoidConfusionAsUnderscoreIsNotAllowed;
	@track urlGMap;
	constructed = false;
	...
	@api
	get mapData(){
		return this.mapDataLocalName;
	}
	set mapData(value){
		this.mapDataLocalName = value;
		if(this.constructed)this.sendToVF(false);
	}
	@api
	get companyTypeFilter(){
		return this.companyTypeFilterLocalNameToAvoidConfusionAsUnderscoreIsNotAllowed;
	}
	set companyTypeFilter(value){
		this.companyTypeFilterLocalNameToAvoidConfusionAsUnderscoreIsNotAllowed = value;
		if(this.constructed)this.sendToVF(false);
	}
	...
	sendToVF (loadMapScript, initMap = true) {
		...
		var message = {
			...
			"initMap" : initMap,
			"companyTypeFilter" : this.companyTypeFilter && this.companyTypeFilter !== 'All' ? this.companyTypeFilter.replaceAll(" ", "_") : null
		} ;
		//Send message to VF
