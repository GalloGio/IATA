...
export default class TidsVetting extends NavigationMixin(LightningElement) {
	...
	pickVetting(event) {
		...
		let caseid = event.target.dataset.targetId;
		assignToCaseOwner( {caseId: caseid}).then(results => {
			if (results.hasAnError===false){
				this.runVetting(caseid);
			}else{
				this.modalDefaultMessage='The ownership transfer is not allowed for this case for this reason: '+results.message;
				this.modalAction='OK';
	...
	getFilteredTidsCases(myfilter) {
		...
		this.spinner = true;
		console.log('My');
		allMyTidsCases({filter:myfilter}).then(results => {
	...
	startVetting(event){
		...
		this.showConfimationModal=false;
		let caseid = event.target.dataset.targetId;
		actionApplication({caseId:caseid, action:'vetting'}).then(result => {
			if (result==='error'){
				this.spinner = false;
				this.modalDefaultMessage='This Application/Request was either recalled by the applicant or is currently being processed by an IATA Staff. Please refresh your screen to see the current status of this Application/Request.';
				this.modalAction='OK';
				this.showConfimationModal=true;
			}else{
				this.runVetting(caseid);
	...
	mappingCases(payload){    
		payload.forEach(item => {
			...
			if (item.Reason1__c==='TIDS – Relinquishment'
			 || item.Reason1__c==='TIDS – Reinstatement'
			 || item.Reason1__c==='TIDS – VB Service Request'){
					item.allowvetting=false;
			 }

			if (caseStatus ==='approved' || caseStatus==='draft' || caseStatus==='closed'  || caseStatus==='pending customer'){
					item.startvetting=false;
