...
export default class PortalTipReports extends LightningElement {
											...
											if(this.datasetId) {
												...
												getEmbedToken({accessToken: this.accessToken, userId: userId, groupId: this.groupId, reportId: this.reportId, datasetId: this.datasetId, conf: this.powerBiConfig})
													.then(result => {
														...
														this.createSrcAddress(this.reportId, this.groupId, this.embedToken, this.reportId, this.powerBiConfig, this.datasetId, this.embedTokenExpiration);
    ...
    createSrcAddress(reportId, groupId, accessToken, objectId, conf, datasetId, expiration) {
		let address= '/TipPowerBiPage?embedUrl='+encodeURIComponent(conf.Report_Resource__c + '?reportId=' + reportId
