...
trigger trgBankCodeCheckUnique on ICCS_Bank_Account__c (before insert, before update) {
    ...
    List<ICCS_Bank_Account__c> tmpBankAccounts = [SELECT Id, Name FROM ICCS_Bank_Account__c];
    ...
    for (ICCS_Bank_Account__c iba : Trigger.new) {
        if ((Trigger.isInsert && mapBankAccountIdsPerAccountCodes.get(iba.Name) != null) ||
            (Trigger.isUpdate && mapBankAccountIdsPerAccountCodes.get(iba.Name) != null && mapBankAccountIdsPerAccountCodes.get(iba.Name) != iba.Id) ) {
