 ...
 * This trigger is used to assign business hours to SIS case
 * and check if the case is created during non-business hours.
 * If the case is created during non-business hours, update Non-BusinessHoursCaseCreation = true
 * Created By Anita Pai
 * Created On 2-May-2011
 **/
trigger trgCheckBusinessHoursBeforeInsert on Case(before insert, before update)
{
    ...
    {
        if (((newCaseObj.RecordTypeId != null) && newCaseObj.RecordTypeId == sisHelpDeskCaseRecordTypeID) || ((newCaseObj.description != null) && (newCaseObj.description.contains(Label.Case_Area_ICH) || newCaseObj.description.contains(Label.Case_Area_SIS))) || ((newCaseObj.CaseArea__c != null) && (newCaseObj.CaseArea__c == Label.SIS || newCaseObj.CaseArea__c == Label.ICH)))
        {
            hasOneSISCase = true;
            break;
        }
    ...
    if (hasOneSISCase)
    {
        ...
        boolean caseDay = false;
        boolean caseHoliday = false;
        BusinessHours bHour = null;
        list < string > AccountNames = new list < string >
        {
            'ACH', 'AIA', 'IATA - ICH', 'IATA - iiNET', 'IATA ITS DR Escalations', Label.ICH_Application_Support, Label.ICH_Help_Desk_Agent, 'Kale Application Support', Label.SIS_Help_Desk_Agent, Label.SIS_L2_Customer_Support, Label.SIS_Operations, Label.Trust_Weaver
        };
        ...
        if (!accIdList.isEmpty())
        {
            for (Account relatedAccount: accIdList)
            {
                ...
                AccountMap.put(relatedAccount.Name, relatedAccount);
            }
        }
        ...
        if (!ctcIdList.isEmpty())
        {
            ...
            {
                ContactMap.put(relatedContact.accountid, relatedContact);
            }
            for (Contact relatedContact: ctcIdList)
            {
                if (relatedContact.Email != null)
                {
                    if (relatedContact.Email.equals(System.Label.KALE_SUPPORT_EMAIL)) ContactMap.put(relatedContact.accountid, relatedContact);
                }
            }
        ...
        List < ExcludedEmailFromAddress__c > ListOfExcludedAddress = [SELECT Name, ApplicationName__c From ExcludedEmailFromAddress__c WHERE ApplicationName__c = : Label.ANY
            OR ApplicationName__c = : Label.SIS
        ];
        List < ExcludedCaseSubject__c > ListOfExcludedSubjectd = [SELECT Name, Application_Name__c From ExcludedCaseSubject__c WHERE Application_Name__c = : Label.ANY
            OR Application_Name__c = : Label.SIS
        ];
        ...
        for (Case newCaseObj: trigger.new)
        {
            if ((trigger.isBefore && Trigger.isInsert) && ((newCaseObj.CaseArea__c != null && newCaseObj.CaseArea__c == Label.SIS) || (newCaseObj.description != null && newCaseObj.description.contains(Label.Case_Area_SIS)) || (newCaseObj.CaseArea__c != null && newCaseObj.CaseArea__c == Label.ICH) || (newCaseObj.RecordTypeId != null && newCaseObj.RecordTypeId == sisHelpDeskCaseRecordTypeID)))
            {
        ...
        if (trigger.isBefore)
        {
            if (Trigger.isInsert)
            {
                ...
                for (Case newCaseObj: trigger.new)
                {
                    ...
                    if (newCaseObj.CaseArea__c != null && newCaseObj.CaseArea__c == Label.SIS && newCaseObj.Status == Label.Escalated && (newCaseObj.Assigned_To__c != Label.SIS_Help_Desk_Agent && newCaseObj.Assigned_To__c != Label.SIS_Operations && newCaseObj.Assigned_To__c != Label.SIS_L2_Customer_Support))
                    {
                        //modified July 2014 to lower SOQL queries

                        Account theAccount = AccountMap.get(newCaseObj.Assigned_To__c);
                        if (theAccount <> null)
                        {
                        ...
                        Contact theContact = ContactMap.get(newCaseObj.L2_Support_Team_Account__c);
                        if (theContact <> null) newCaseObj.L2_L3_Support_Owner__c = string.valueOf(theContact.Id);
                    }
                    ...
                    /*ICH web-to-case*/
                    if ((newCaseObj.CaseArea__c == Label.ICH && newCaseObj.RecordTypeId == null) || (newCaseObj.CaseArea__c == Label.ICH && newCaseObj.RecordTypeId == sisHelpDeskCaseRecordTypeID) || (newCaseObj.description != null && newCaseObj.description.contains(Label.Case_Area_ICH)))
                    {
                        ...
                        if (newCaseObj.suppliedemail != null && newCaseObj.suppliedemail != EscalatedCaseHelper.KALE_SUPPORT_EMAIL && newCaseObj.description != null && newCaseObj.description.contains(Label.Case_Area_ICH))
                        {
                    ...
                    /* 
                       K.A 9-11-2014 : IS email to case
                       E-mail to Case - IS Help Desk
                    */
                    if ((newCaseObj.Origin == Label.E_mail_to_Case_IS_Help_Desk || newCaseObj.Origin == Label.E_mail_to_Case_SIS_Help_Desk )&& newCaseObj.RecordTypeId != null)
                    {
                        ...
                        boolean isIS = false ;
                        if (newCaseObj.SuppliedEmail !=System.Label.KALE_SUPPORT_EMAIL && newCaseObj.Origin != Label.E_mail_to_Case_SIS_Help_Desk)  isIS = clsCheckBusinessHours.ISCaseIdentifyAndProcess(newCaseObj);
                        ...
                        if (isIS || newCaseObj.SuppliedEmail ==System.Label.KALE_SUPPORT_EMAIL || newCaseObj.Origin == Label.E_mail_to_Case_SIS_Help_Desk )
                        {
                            ...
                            if (newCaseObj.SuppliedEmail ==System.Label.KALE_SUPPORT_EMAIL)
                            {
                                ...
                                {
                                    if (sfCaseNumber == null) throw new EmailParsingException(Label.EmailParsingExceptionKaleNumber);
                                ...
                                if (EscalatedCaseHelper.caseHasBeenClosedByKale(newCaseObj) || EscalatedCaseHelper.caseStatusHasBeenUpdatedByKale(newCaseObj))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (thisfCaseNumber == null || thisfCaseNumber.trim().length() == 0) throw new EmailParsingException(Label.EmailParsingExceptionKaleNumber);
                                        ...
                                        if (newStatus == null) throw new EmailParsingException(Label.EmailParsingExceptionKaleStatus);
                                        ...
                                        if (newStatus.trim() == 'Onhold') newStatus = 'On hold';
                            ...
                            if (newCaseObj.SuppliedEmail != null && newCaseObj.SuppliedEmail != EscalatedCaseHelper.KALE_SUPPORT_EMAIL)
                            {
                                ...
                                {
                                    parentCaseNumber = [Select c.id, Kale_Status__c, c.CaseNumber, c.Subject, ownerid, SuppliedEmail From Case c where
                                        c.RecordTypeId != null
                                        and c.RecordTypeId = : sisHelpDeskCaseRecordTypeID
                                        and c.SuppliedEmail != null
                                        and c.Origin != null
                                        and c.Subject != null
                                        and c.ownerid != null
                                        //and (owner.name like '%SIS Help Desk%')
                                        and c.Origin = :Label.E_mail_to_Case_IS_Help_Desk
                                    ];
                                    if (!parentCaseNumber.isEmpty())
                                    {
            ...
            if (trigger.isUpdate)
            {
                // Initialisation of variables for Before Update.

                String Kale_BusinessHours_Name = Label.SIS_HelpDesk_Kale;
                String SIS_BusinessHours_Name = Label.SIS_Help_Desk;
                Set < String > casePriorityAllowed = new Set < String >
                {
                    Label.Priority_1_Showstopper, Label.Priority_2_Major, Label.Priority_3_Minor, Label.Priority_4_Trivial_Defects
                };
                ...
                for (Case caseObjNew: Trigger.new)
                {
                    Case caseObjOld = Trigger.oldMap.get(caseObjNew.Id);

                    // To update L2 Support team details whenever a case is escalated               
                    if (caseObjOld.casearea__C != null && caseObjOld.CaseArea__c == Label.SIS && caseObjNew.Status == Label.Escalated && caseObjOld.Assigned_To__c != caseObjNew.Assigned_To__c && (caseObjNew.Assigned_To__c != Label.ICH_Application_Support && caseObjNew.Assigned_To__c != Label.SIS_Help_Desk_Agent && caseObjNew.Assigned_To__c != Label.SIS_Operations && caseObjNew.Assigned_To__c != Label.SIS_L2_Customer_Support))
                    {
                        Account theAccount = AccountMap.get(caseObjNew.Assigned_To__c);
                        if (theAccount <> null)
                        {
                        ...
                        Contact theContact = ContactMap.get(string.valueOf(caseObjNew.L2_Support_Team_Account__c));
                        if (theContact <> null) caseObjNew.L2_L3_Support_Owner__c = string.valueOf(theContact.Id);
                        system.debug('\ncaseObjNew.L2_L3_Support_Owner__c: ' + caseObjNew.L2_L3_Support_Owner__c);
                    }
                    else
                    {
                        if (caseObjOld.CaseArea__c == Label.SIS && caseObjNew.Status == Label.Escalated && caseObjOld.Assigned_To__c != caseObjNew.Assigned_To__c && (caseObjNew.Assigned_To__c == Label.SIS_Help_Desk_Agent || caseObjNew.Assigned_To__c == Label.SIS_Operations || caseObjNew.Assigned_To__c == Label.SIS_L2_Customer_Support))
                        {
                    ...
                    if (caseObjNew.CaseArea__c == Label.SIS && caseObjNew.RecordTypeId == sisHelpDeskCaseRecordTypeID)
                    {
                        ...
                        if (caseObjOld.HasSISCaseBeenClosed__c == false && SISHelpDesk_CaseSLATracking.KaleStopStatus.contains(caseObjOld.Kale_Status__c) == false && SISHelpDesk_CaseSLATracking.KaleStopStatus.contains(caseObjNew.Kale_Status__c) == true)
                        {
                        ...
                        if (caseObjOld.HasSISCaseBeenClosed__c == false && SISHelpDesk_CaseSLATracking.SISStopStatus_FromKale.contains(caseObjOld.Kale_Status__c) == false && SISHelpDesk_CaseSLATracking.SISStopStatus_FromKale.contains(caseObjNew.Kale_Status__c) == true)
                        {
                        ...
                        if (caseObjOld.HasSISCaseBeenClosed__c == false && SISHelpDesk_CaseSLATracking.SISStopStatus.contains(caseObjOld.Status) == false && SISHelpDesk_CaseSLATracking.SISStopStatus.contains(caseObjNew.Status) == true)
                        {
                            SISHelpDesk_CaseSLATracking.updateSLA_SIS_BeginStopStatus(caseObjNew, sisBusinessHours);
                        }
                        ...
                        if (caseObjOld.HasSISCaseBeenClosed__c == false && SISHelpDesk_CaseSLATracking.KaleStopStatus.contains(caseObjOld.Kale_Status__c) == true && SISHelpDesk_CaseSLATracking.KaleStopStatus.contains(caseObjNew.Kale_Status__c) == false)
                        {
                        ...
                        List < Case > c = SISHelpDesk_CaseSLATracking.updateSLA(new List < Case >
                        {
                            caseObjNew
                        }, sisBusinessHours, kaleBusinessHours);
                        if (c.size() > 0)
                        {
