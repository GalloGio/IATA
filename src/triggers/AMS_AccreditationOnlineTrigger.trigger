trigger AMS_AccreditationOnlineTrigger on AMS_Pax_Accreditation_Form__c (before insert, before update) {
    ...
    if(trigger.isInsert) {
        ...
        AMS_AccreditationOnlineTriggerHelper.prepopulateBillingIsoState(trigger.new, null);
		AMS_AccreditationOnlineTriggerHelper.fillState(Trigger.new, null);
    }
    else if(trigger.isUpdate) {
        for(AMS_Pax_Accreditation_Form__c ac : trigger.new)
        	AMS_AccreditationOnlineTriggerHelper.prepopulateAbbreviatedFields(ac, trigger.oldMap.get(ac.id));

    	AMS_AccreditationOnlineTriggerHelper.prepopulateBillingIsoState(trigger.new, trigger.oldMap);
		AMS_AccreditationOnlineTriggerHelper.fillState(Trigger.new, Trigger.oldMap);
    }
}