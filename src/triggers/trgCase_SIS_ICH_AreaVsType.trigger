trigger trgCase_SIS_ICH_AreaVsType on Case(before insert, before update) {
    ...
    case newCase:
        ...
        if (newCase.Type != null && newCase.CaseArea__c != null) {
            if (newCase.CaseArea__c == 'ICH' && (newCase.Type == 'Feature Request' || newCase.Type == 'General Question' || newCase.Type == 'Problem / Issue')) { //system.debug('\n case area:' + newCase.CaseArea__c + ' type: ' + newCase.Type );  
                system.debug('\n Assert error2 caught .......');
                newCase.addError(Label.HelpDesk_SIS_ICH_Type_Area_Mismatch);
            }

            if (newCase.CaseArea__c == 'SIS' && (newCase.Type != 'Feature Request' && newCase.Type != 'General Question' && newCase.Type != 'Problem / Issue')) { //system.debug('\n case area:' + newCase.CaseArea__c + ' type: ' + newCase.Type);
