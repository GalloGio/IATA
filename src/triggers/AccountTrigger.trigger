trigger AccountTrigger on Account (before insert, after insert, after update, before update, before delete, after delete, after undelete) {
	...
	if (!AMS_TriggerExecutionManager.checkExecution(Account.getSObjectType(), 'AccountTrigger')) {
		return;
	}
	...
	if (Trigger.isBefore && Trigger.isInsert) angHandler.onBeforeInsert(); else if (Trigger.isAfter && Trigger.isInsert) angHandler.onAfterInsert(); else if (Trigger.isBefore && Trigger.isUpdate) angHandler.onBeforeUpdate(); else if (Trigger.isAfter && Trigger.isUpdate) angHandler.onAfterUpdate(); else if (Trigger.isBefore && Trigger.isDelete) angHandler.onBeforeDelete(); else if (Trigger.isAfter && Trigger.isDelete) angHandler.onAfterDelete();
