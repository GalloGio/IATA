            ...
            if(!casesToTrigger.isEmpty()){
                ...
                for (Case c : casesToTrigger){
                    ...
                    if(c.Reason1__c.startsWith('PASS')){
                        ...
                        //PREIVIOUS -->//This prevents the parent case to be closed before closing the child cases. Only used in the PASS Airline Participation
                //MMSA
                //WANT TO PREVENT THE CHILD CASES TO BE CLOSED BEFORE CLOSING THE PARENT CASE
                if(checkChildCasesId.size()>0){
                    List<Case> closedChildCaseList = [SELECT Id,CaseNumber,ParentId, Parent.Status, Parent.caseNumber 
                                                                FROM Case 
                                                                WHERE /*ParentId*/ Id In:checkChildCasesId AND Status = 'Closed' AND ParentId != null]; //all closed child cases

                    Map<Id,String> notClosedParentCaseParentMap= new Map<Id,String>();

                    /*for(Case openedChildCase:notClosedParentCaseList){
                    ...
                    /*for(Case childCase : closedChildCaseList) {
                        if(childCase.Parent.Status != 'Closed') { //if Status of parent case IS NOT Closed, we add the Case to the Map
                            notClosedParentCaseParentMap.put(childCase.Id,childCase.CaseNumber); //(parentCaseID,childCaseNumber)
                        }
                    }*/

                    /*for (Case c : casesToTrigger){
                        if(notClosedParentCaseParentMap.containsKey(c.Id)){ //If map has the caseId --> ERROR, since parentCase IS NOT Closed
                            
                            for(Case cc : closedChildCaseList) {
                                c.addError('Please close the following parent case before closing this case: ' + cc.Parent.caseNumber + '.');
                            }
                        }
                    }*/
                }
...
/*Trigger.isUpdate*/
    ...
    if(trigger.isAfter && Trigger.isUpdate) {
        Map<Id, AP_Process_Form__c> casePassCountryMap = new Map<Id, AP_Process_Form__c>(); //vai conter os cases (é suposto ter só os parents - reverificar validação) ao qual iremos enviar PE

        List<Case> closedParentCaseList = [SELECT Id,CaseNumber,ParentId, Status
                                                                FROM Case 
                                                                WHERE Status = 'Closed' AND ParentId = null AND  Reason1__c LIKE 'PASS Participation%' AND Id IN:Trigger.new]; //all closed parent cases, which are first time airline is accessing 'join settlement service' 
        System.debug('MMSA:: closedparentcaselist: '+closedparentcaselist);

        List<AP_Process_Form__c> apFormList = [SELECT Id, RecordTypeId FROM AP_Process_Form__c WHERE Case__c IN:closedParentCaseList];
        System.debug('MMSA:: apformlist: '+apformlist);

        //Map<Id, AP_Process_Form__c> apFormMap 

        //for (Services_Rendered__c srRecord : Trigger.new) {
            for(Case c : Trigger.new) {
                Case oldCase = Trigger.oldMap.get(c.Id);
                if(/*srRecord.Services_Rendered_Type__c == 'PASS Country' && */c.Status == 'Closed' && oldCase.Status != 'Closed'){
                    for(AP_Process_Form__c form : apFormList) {
                        //casePassCountryMap.put(c.Account.AP_Process_Form__c,c);//-----------------------HERE---------------------------
                        casePassCountryMap.put(form.Id,form);
                    }
                }
            }     
            System.debug('MMSA::casePassCountryMap: '+casePassCountryMap); 
        //}
   
        if(casePassCountryMap != null && casePassCountryMap.size() > 0) {
            System.debug('MMSA:: first IF got in');

            if((Limits.getLimitQueueableJobs() - Limits.getQueueableJobs()) > 0 && !System.isFuture() && !System.isBatch()) {
                System.debug('MMSA:: second IF got in');
                System.enqueueJob(new PlatformEvents_Helper(casePassCountryMap, 'Airline_Account__e', 'AP_Process_Form__c', trigger.isInsert, true, trigger.isDelete, trigger.isUndelete)); //trigger.isInsert, trigger.isUpdate, trigger.isDelete, trigger.isUndelete
            } else {
                System.debug('MMSA::else got in');
