public class ANG_AccountTriggerHandler {
	...
	private void manageDowngrades() {
		...
		for(Account acc : triggerNew){
			...
			if(acc.ANG_HE_RiskStatus__c == 'B' && oldMap.get(acc.Id).ANG_HE_RiskStatus__c == 'A' && acc.ANG_CashCondition__c == oldMap.get(acc.Id).ANG_CashCondition__c) downGradeAtoB.add(acc);
			if(acc.ANG_HE_RiskStatus__c == 'C' && oldMap.get(acc.Id).ANG_HE_RiskStatus__c == 'B') downGradeBtoC.add(acc);
		}
		...
		for(Contact c : [SELECT Id, AccountId FROM Contact WHERE User_Portal_Status__c = 'Approved Admin' AND (AccountId IN :downGradeAtoB OR AccountId IN :downGradeBtoC)]){
			portalAdmins.put(c.AccountId, c.Id);
		}

		//create cases
		List<Case> newCases = new List<Case>();
		Id saamRecordType = RecordTypeSingleton.getInstance().RtIDsPerDeveloperNamePerObj.get('Case').get('ProcessEuropeSCE');
		...
		for(Account acc : downGradeAtoB){
			...
			c.Origin = 'Internal case';
			c.CaseArea__c = 'Risk Management Process'; 
			c.Reason1__c = 'Financial Security Update';
	...
	private void createVMFROscars(List<Account> allAccounts, Map<Id, Id> contacts){
		...
		for(Account acc : accounts){
			...
			AMS_OSCAR__c oscar = new AMS_OSCAR__c(RecordTypeId = AMS_Utils.RECTYPE_NGCHANGES, Account__c = acc.Id,  Process__c = AMS_Utils.NGCHANGES, Status__c='Open', Type_of_change__c = AMS_Utils.VFMR);
			oscars.add(oscar);
		...
		for(Account acc : accounts){
			...
			c.BSPCountry__c = acc.BillingCountry;
			c.Region__c = acc.Region_formula__c;
			c.CaseArea__c = 'Accreditation Process';
			c.Reason1__c = 'Change of '+AMS_Utils.VFMR;
	...
	public void UpdateRiskEventStatus (){
		...
		List<ANG_Agency_Risk_Event__c> reToUpdateList=[select Id,ang_event_status__c,ANG_Issued_Date__c,ANG_AccountId__c from ANG_Agency_Risk_Event__c where ANG_AccountId__c in :accIdSet and  ang_event_status__c in ('Active','Inactive','Inactive_TER')];
		
		for(ANG_Agency_Risk_Event__c re : reToUpdateList){
			if(inactiveAccsIdSet.contains(re.ANG_AccountId__c) 
				&& (re.ang_event_status__c == 'Active' || (re.ang_event_status__c == 'Inactive' && re.ANG_Issued_Date__c > system.Today()))){
			        re.ang_event_status__c = 'Inactive_TER';
				
			} else if (activeAccsIdSet.contains(re.ANG_AccountId__c) && re.ang_event_status__c == 'Inactive_TER'){
		        re.ang_event_status__c = 'Active';
			}
    ...
    private void createAgencyOperations() {

    	List<Account> accountsToLink;
    	Set<Id> countryIds = new Set<Id>();

    	for(Account acc: triggerNew) {

    		if(acc.ANG_IsNewGenAgency__c && acc.IATACode__c != null && oldMap.get(acc.id).IATACode__c == null && acc.RecordTypeId == ANG_ConversionHelper.ACCOUNT_AGENCY_RT && oldMap.get(acc.id).RecordTypeId == ANG_ConversionHelper.ACCOUNT_STANDARD_RT){   			
    			accountsToLink.add(acc);
    			countryIds.add(acc.IATA_ISO_Country__c);
    		}
    	}

    	if(!accountsToLink.isEmpty()) {
    		List<AMS_Operation__c> auxList;
    		List<AMS_Operation__c> operationCodes = [SELECT Id, Country__c FROM AMS_Operation__c WHERE Country__c in :countryIds AND Settlement__r.RecordType.Name = 'BSP'];

    		//we can have more than 1 operation code per country so we'll put them on a map
    		Map<Id,List<AMS_Operation__c>> operationCodesPerCountry = new Map<Id,List<AMS_Operation__c>>();

    		for(AMS_Operation__c operationCode: operationCodes) {
    			if (operationCodesPerCountry.Keyset().contains(operationCode.Country__c)) {   				
    				operationCodesPerCountry.get(operationCode.Country__c).add(operationCode);
    			}else {
    				auxList = new List<AMS_Operation__c>();
    				auxList.add(operationCode);
    				operationCodesPerCountry.put(operationCode.Country__c, auxList);   				
    			}
    		}

    		if(!operationCodesPerCountry.isEmpty()){
    			AMS_Agency_Operations__c agencyOp;
    			List<AMS_Agency_Operations__c> agencyOpsToInsert = new List<AMS_Agency_Operations__c>();
    			...
    			if(!agencyOpsToInsert.isEmpty()){
    				insert agencyOpsToInsert;
    			}
    		}
    	}
    }
