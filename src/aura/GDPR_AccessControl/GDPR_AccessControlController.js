({
	doInit : function(component, event, helper) {
		...
		console.log('Param sURLVariables '+sURLVariables);
		
		 
		for (var i = 0; i < sURLVariables.length; i++) {
			
			sParameterName = sURLVariables[i].split('='); //to split the key from the value.
			console.log('KEY - '+sParameterName[0]);
		...
		console.log('encryptedIndividualId - '+encryptedIndividualId);
		...
		if(encryptedToken && encryptedIndividualId){
			...
			action.setCallback(this, function(response){
				...
				console.log('JSON - '+responseValue);
