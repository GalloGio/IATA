({
	...
	loadGadmAccount : function(component, event, gadmAccountId) {
	    let action = component.get('c.loadGadmAccount');
	    action.setParams({
	        'gadmAccountId' : gadmAccountId
        });
        action.setCallback(this, function(response){
            const state = response.getState();
            if(state === 'SUCCESS') {
                const account = response.getReturnValue();
                if(! $A.util.isEmpty(account.Id)) {
                    component.set('v.account', account);
                    component.set('v.account.Name', account.Name);
                    component.set('v.gadmAccountSelected', true);

                }else{
                    component.set('v.gadmAccountId', '');
                    console.log('empty account returned');
                }
            }else{
                component.set('v.gadmAccountId', '');
                console.log('loadGadmAccount error');
            }
        });
