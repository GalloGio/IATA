		...
		checkemail.setCallback(this, function(response) {
				...
				if (state === "SUCCESS") {
					if (response.getReturnValue() === false) {
						...
						emailField.set("v.errors", [{message: $A.get("$Label.c.ISSP_Registration_UserExist") }]);
...
// console.log(component.find("ms").get("v.value"));
		...
		if(membershipFunction === undefined) {
		...
		if(businessphone === undefined) {
			console.log('bp');
			businessphoneField.set("v.errors", [{message: $A.get("$Label.c.ISSP_Registration_BusinessPhone") }]);
			// errors = true;
