({
	...
	getIndustryActivities : function(component) {
		var action = component.get("c.getIndustryActivities");
		var accountId = component.get("v.accountId");
		...
		action.setParams({
			...
			action.setCallback(this, function(a) {
				 ...
				 if (component.isValid() && state === "SUCCESS") {
					var industryActivitiesWrappers = new Array();
					for(var i = 0; i < services.length; i++) {
						var rowspan = 1;
						if(i+1 < services.length) {
							for(var j=i+1; j < services.length; j++) {
						...
						industryActivitiesWrappers.push(sWrapper);
					}
					console.log(JSON.stringify(industryActivitiesWrappers));
					component.set("v.AccountIndustryActivitiesWrapper", industryActivitiesWrappers);
				 }
				 else if (state === "ERROR") {}
			});
			$A.enqueueAction(action);
	},
    isIATAMember : function(component) {
		var action = component.get("c.isIATAMember");
		var accountId = component.get("v.accountId");
        ...
        action.setCallback(this, function(actionResult) {
            // console.log(JSON.stringify(actionResult.getReturnValue()));
            component.set("v.isIATAMember", actionResult.getReturnValue());
        });
        $A.enqueueAction(action);
	}
})